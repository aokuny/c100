...
public class QuoteThreadPicc extends Thread{
	...
	public void run() {
		try {
			...
			param = this.setHebaoParam(param, LicenseNo, (String) carMap.get("identifyNumber"));
...
//			System.err.println("辅助核保计算结果：" + html);
	 ...
	 * @return
	 */
	private String setHebaoParam(String param, String carNo, String idCode){
		...
		//第六部分：设置险种
		JSONArray prpCitemKindsArray = syxJson.getJSONArray("data").getJSONObject(0).getJSONArray("prpCitemKinds");
		...
		param = param +  "&sumPayTax1=" + QuoteCalculateUtils.mN(ciCarShipTax.getString("thisPayTax"), 2);
		param = param.replace("prpCstampTaxCI.ciPayTax=", "prpCstampTaxCI.ciPayTax=0");
		
		Double sumPremium1 = 0D;
		sumPremium1 = Double.parseDouble(QuoteCalculateUtils.mN(String.valueOf(sumPremium), 2)) + Double.parseDouble(QuoteCalculateUtils.mN(String.valueOf(ciPremium), 2));
		...
		String code = "1100100001900281";
		
		String tName = "朱佳佳";
		try {
			tName = java.net.URLEncoder.encode(tName, "gb2312");
		} catch (Exception e) {
			e.printStackTrace();
		}
		String tIdCode = "239005198802252314";
		String tMobile = "13520030193";
		
		// 投保人 被保人
		param = param.replace("iinsuredCode=001", "iinsuredCode=" + code);//code
		param = param.replace("prpCinsureds_%5B0%5D.insuredCode=001","prpCinsureds_%5B0%5D.insuredCode=" + code);
		param = param.replace("prpCinsureds%5B1%5D.insuredCode=", "prpCinsureds%5B3%5D.insuredCode=" + code);
		param = param.replace("iinsuredName=001", "iinsuredName=" + tName);//名字
		param = param.replace("prpCinsureds%5B1%5D.insuredName=%D6%EC%BC%D1%BC%D1","prpCinsureds%5B1%5D.insuredName=" + tName);
		param = param.replace("prpCinsureds_%5B0%5D.insuredName=1","prpCinsureds_%5B0%5D.insuredName=" + tName);
		param = param.replace("iidentifyNumber=001", "iidentifyNumber=" + tIdCode);//身份证
		param = param.replace("prpCinsureds_%5B0%5D.identifyNumber=1","prpCinsureds_%5B0%5D.identifyNumber="+ tIdCode);
		param = param.replace("prpCinsureds_%5B0%5D.drivingLicenseNo=","prpCinsureds_%5B0%5D.drivingLicenseNo=" + tIdCode);
		param = param.replace("prpCinsureds%5B1%5D.identifyNumber=320684198411040279","prpCinsureds%5B3%5D.identifyNumber=" + tIdCode);
		param = param.replace("prpCinsureds%5B1%5D.drivingLicenseNo=","prpCinsureds%5B3%5D.drivingLicenseNo=" + tIdCode);
		param = param.replace("iinsuredAddress=001", "iinsuredAddress=");//地址
		param = param.replace("prpCinsureds%5B1%5D.insuredAddress=...", "prpCinsureds%5B3%5D.insuredAddress=");
		param = param.replace("prpCinsureds_%5B0%5D.mobile=1", "prpCinsureds_%5B0%5D.mobile=" + tMobile);//手机
		param = param.replace("prpCinsureds%5B1%5D.phoneNumber=13520030193","prpCinsureds%5B1%5D.phoneNumber=" + tMobile);
		param = param.replace("prpCinsureds%5B1%5D.drivingYears=", "prpCinsureds%5B3%5D.mobile=" + tMobile);
		param = param.replace("mobile%5B1%5D=189****0047", "imobile=" + tMobile);
		param = param.replace("imobile=001", "imobile=" + tMobile);
		param = param.replace("prpCinsureds_%5B0%5D.age=", "prpCinsureds_%5B0%5D.age=");//年龄
		param = param.replace("prpCinsureds%5B1%5D.age=", "prpCinsureds%5B3%5D.age=");
		
		String cName = "朱佳佳";
		try {
			cName = java.net.URLEncoder.encode(cName, "gb2312");
		} catch (Exception e) {
			e.printStackTrace();
		}
		String cIdCode = "320684198411040279";
		String cMobile = "13520030193";
		
		//车主信息
		param = param.replace("prpCinsureds%5B0%5D.insuredCode=", "prpCinsureds%5B2%5D.insuredCode=" + code);//code
		param = param + "&iinsuredCode=" + code;
		param = param.replace("prpCinsureds%5B0%5D.insuredName=%D6%EC%BC%D1%BC%D1","prpCinsureds%5B2%5D.insuredName=" + cName);//名字
		param = param + "&iinsuredName=" + cName;
		param = param.replace("prpCinsureds%5B0%5D.identifyNumber=320684198411040279","prpCinsureds%5B2%5D.identifyNumber=" + cIdCode);//身份证
		param = param + "&iidentifyNumber=" + cIdCode;
		param = param.replace("prpCinsureds%5B0%5D.drivingLicenseNo=","prpCinsureds%5B2%5D.drivingLicenseNo=" + cIdCode);
		param = param.replace("prpCinsureds%5B0%5D.insuredAddress=...","prpCinsureds%5B2%5D.insuredAddress=");//地址
		param = param + "&iinsuredAddress=";
		param = param.replace("prpCinsureds%5B0%5D.drivingYears=", "prpCinsureds%5B2%5D.mobile=" + cMobile);//手机
		param = param.replace("mobile%5B0%5D=189****0047", "imobile=" + cMobile);
		param = param.replace("prpCinsureds%5B0%5D.age=", "prpCinsureds%5B2%5D.age=");
		
		
		param = param.replace("prpCinsureds_%5B0%5D.insuredFlag=1","prpCinsureds_%5B0%5D.insuredFlag=11000000000000000000000000000A");
		param = param.replace("iinsuredFlag=001", "iinsuredFlag=%CD%B6%B1%A3%C8%CB/%B1%BB%B1%A3%CF%D5%C8%CB");
		param = param.replace("iinsuredType=001", "iinsuredType=%B8%F6%C8%CB");
		param = param.replace("iunitType=001", "iunitType=");
		param = param.replace("iidentifyType=001", "iidentifyType=%C9%ED%B7%DD%D6%A4");
		param = param.replace("iemail=001", "iemail=");
		param = param.replace("iphoneNumber=001", "iphoneNumber=");
		param = param.replace("prpCinsureds_%5B0%5D.unitType=1", "prpCinsureds_%5B0%5D.unitType=");
		param = param.replace("prpCinsureds_%5B0%5D.identifyType=1", "prpCinsureds_%5B0%5D.identifyType=01");
		param = param.replace("prpCinsureds_%5B0%5D.email=1", "prpCinsureds_%5B0%5D.email=");
		param = param.replace("prpCinsureds_%5B0%5D.phoneNumber=1", "prpCinsureds_%5B0%5D.phoneNumber=");
		param = param.replace("prpCinsureds_%5B0%5D.postCode=1", "prpCinsureds_%5B0%5D.postCode=");
		param = param.replace("prpCinsureds_%5B0%5D.versionNo=1", "prpCinsureds_%5B0%5D.versionNo=3");
		param = param.replace("prpCinsureds_%5B0%5D.auditStatus=1", "prpCinsureds_%5B0%5D.auditStatus=2");
		param = param.replace("prpCinsureds_%5B0%5D.countryCode=1", "prpCinsureds_%5B0%5D.countryCode=CHN");
		param = param.replace("idCardCheckInfo_%5B0%5D.flag=", "idCardCheckInfo_%5B0%5D.flag=0");
		param = param.replace("iauditStatus=001", "iauditStatus=2");
		param = param.replace("iversionNo=001", "iversionNo=3");
		param = param.replace("prpCinsureds%5B0%5D.insuredFlag=001000000000000000000000000000","prpCinsureds%5B2%5D.insuredFlag=001000000000000000000000000000");
		param = param.replace("display_insuredFlag=%B3%B5%D6%F7", "iinsuredFlag=%B3%B5%D6%F7");
		param = param.replace("prpCinsureds%5B0%5D.id.serialNo=0", "prpCinsureds%5B2%5D.id.serialNo=1");
		param = param.replace("display_InsuredNature=%B8%F6%C8%CB", "");
		param = param.replace("prpCinsureds%5B0%5D.insuredType=1", "prpCinsureds%5B2%5D.insuredType=1");
		param = param.replace("prpCinsureds%5B0%5D.insuredNature=3", "prpCinsureds%5B2%5D.insuredNature=1");
		param = param.replace("unitTypeText=", "");
		param = param.replace("prpCinsureds%5B0%5D.unitType=", "prpCinsureds%5B2%5D.unitType=");
		param = param.replace("display_identifyType=%C9%ED%B7%DD%D6%A4", "");
		param = param.replace("prpCinsureds%5B0%5D.identifyType=01", "prpCinsureds%5B2%5D.identifyType=01");
		param = param.replace("prpCinsureds%5B0%5D.email=", "prpCinsureds%5B2%5D.email=");
		param = param.replace("prpCinsureds%5B0%5D.phoneNumber=13520030193", "prpCinsureds%5B2%5D.phoneNumber=");
		param = param.replace("prpCinsureds%5B0%5D.sex=", "prpCinsureds%5B2%5D.drivingYears=");
		param = param.replace("prpCinsureds%5B0%5D.postCode=100000", "prpCinsureds%5B2%5D.postCode=");
		param = param.replace("prpCinsureds%5B0%5D.versionNo=0", "prpCinsureds%5B2%5D.versionNo=3");
		param = param.replace("prpCinsureds%5B0%5D.auditStatus=", "prpCinsureds%5B2%5D.auditStatus=2");
		param = param.replace("prpCinsureds%5B0%5D.countryCode=CHN", "prpCinsureds%5B2%5D.countryCode=CHN");
		param = param.replace("prpCinsureds%5B0%5D.flag=", "prpCinsureds%5B2%5D.flag=");
		param = param.replace("prpCinsureds%5B0%5D.appendPrintName=", "prpCinsureds%5B2%5D.drivingCarType=");
		param = param.replace("prpCinsureds%5B0%5D.drivingCarType=", "prpCinsureds%5B2%5D.appendPrintName=");
		param = param.replace("reLoadFlag%5B0%5D=true", "");
		param = param.replace("prpCinsureds%5B0%5D.causetroubleTimes=0", "prpCinsureds%5B2%5D.causetroubleTimes=");
		param = param.replace("prpCinsureds%5B0%5D.acceptLicenseDate=", "prpCinsureds%5B2%5D.acceptLicenseDate=");
		param = param.replace("isCheckRepeat%5B0%5D=0", "isCheckRepeat%5B2%5D=");
		param = param.replace("configedRepeatTimes%5B0%5D=", "configedRepeatTimes%5B2%5D=");
		param = param.replace("repeatTimes%5B0%5D=", "repeatTimes%5B2%5D=");
		param = param.replace("prpCinsureds%5B0%5D.unifiedSocialCreditCode=",	"prpCinsureds%5B2%5D.unifiedSocialCreditCode=");
		param = param.replace("idCardCheckInfo%5B0%5D.insuredcode=", "idCardCheckInfo%5B2%5D.insuredcode=");
		param = param.replace("idCardCheckInfo%5B0%5D.insuredFlag=", "idCardCheckInfo%5B2%5D.insuredFlag=");
		param = param.replace("idCardCheckInfo%5B0%5D.mobile=", "idCardCheckInfo%5B2%5D.mobile=");
		param = param.replace("idCardCheckInfo%5B0%5D.idcardCode=", "idCardCheckInfo%5B2%5D.idcardCode=");
		param = param.replace("idCardCheckInfo%5B0%5D.name=", "idCardCheckInfo%5B2%5D.name=");
		param = param.replace("idCardCheckInfo%5B0%5D.nation=", "idCardCheckInfo%5B2%5D.nation=");
		param = param.replace("idCardCheckInfo%5B0%5D.birthday=", "idCardCheckInfo%5B2%5D.birthday=");
		param = param.replace("idCardCheckInfo%5B0%5D.sex=", "idCardCheckInfo%5B2%5D.sex=");
		param = param.replace("idCardCheckInfo%5B0%5D.address=", "idCardCheckInfo%5B2%5D.address=");
		param = param.replace("idCardCheckInfo%5B0%5D.issure=", "idCardCheckInfo%5B2%5D.issure=");
		param = param.replace("idCardCheckInfo%5B0%5D.validStartDate=", "idCardCheckInfo%5B2%5D.validStartDate=");
		param = param.replace("idCardCheckInfo%5B0%5D.validEndDate=", "idCardCheckInfo%5B2%5D.validEndDate=");
		param = param.replace("idCardCheckInfo%5B0%5D.samCode=", "idCardCheckInfo%5B2%5D.samCode=");
		param = param.replace("idCardCheckInfo%5B0%5D.samType=", "idCardCheckInfo%5B2%5D.samType=");
		param = param.replace("idCardCheckInfo%5B0%5D.flag=", "idCardCheckInfo%5B2%5D.flag=");
		param = param.replace("prpCinsureds%5B0%5D.mobile=13520030193", "iauditStatus=2");
		param = param.replace("prpCinsureds%5B1%5D.insuredFlag=11000000000000000000000000000A","prpCinsureds%5B3%5D.insuredFlag=11000000000000000000000000000A");
		param = param.replace("display_insuredFlag=%CD%B6%B1%A3%C8%CB%2F%B1%BB%B1%A3%CF%D5%C8%CB","iinsuredFlag=%CD%B6%B1%A3%C8%CB/%B1%BB%B1%A3%CF%D5%C8%CB");
		param = param.replace("prpCinsureds%5B1%5D.id.serialNo=1", "prpCinsureds%5B1%5D.id.serialNo=1");
		param = param.replace("prpCinsureds%5B1%5D.insuredType=1", "prpCinsureds%5B1%5D.insuredType=1");
		param = param.replace("prpCinsureds%5B1%5D.insuredNature=3", "prpCinsureds%5B1%5D.insuredNature=3");
		param = param.replace("prpCinsureds%5B1%5D.insuredCode=", "prpCinsureds%5B1%5D.insuredCode=");
		param = param.replace("prpCinsureds%5B1%5D.unitType=", "prpCinsureds%5B1%5D.unitType=");
		param = param.replace("prpCinsureds%5B1%5D.identifyType=01", "prpCinsureds%5B1%5D.identifyType=01");
		param = param.replace("prpCinsureds%5B1%5D.email=", "prpCinsureds%5B1%5D.email=");
		param = param.replace("prpCinsureds%5B1%5D.sex=", "prpCinsureds%5B1%5D.sex=");
		param = param.replace("prpCinsureds%5B1%5D.drivingYears=", "prpCinsureds%5B1%5D.drivingYears=");
		param = param.replace("prpCinsureds%5B1%5D.postCode=100000", "prpCinsureds%5B1%5D.postCode=100000");
		param = param.replace("prpCinsureds%5B1%5D.versionNo=0", "prpCinsureds%5B1%5D.versionNo=0");
		param = param.replace("prpCinsureds%5B1%5D.auditStatus=", "prpCinsureds%5B1%5D.auditStatus=");
		param = param.replace("prpCinsureds%5B1%5D.countryCode=", "prpCinsureds%5B1%5D.countryCode=");
		param = param.replace("prpCinsureds%5B1%5D.flag=", "prpCinsureds%5B1%5D.flag=");
		param = param.replace("prpCinsureds%5B1%5D.age=", "prpCinsureds%5B1%5D.age=");
		param = param.replace("prpCinsureds%5B1%5D.drivingLicenseNo=", "prpCinsureds%5B1%5D.drivingLicenseNo=");
		param = param.replace("prpCinsureds%5B1%5D.appendPrintName=", "prpCinsureds%5B1%5D.appendPrintName=");
		param = param.replace("prpCinsureds%5B1%5D.drivingCarType=", "prpCinsureds%5B1%5D.drivingCarType=");
		param = param.replace("prpCinsureds%5B1%5D.causetroubleTimes=0", "prpCinsureds%5B1%5D.causetroubleTimes=0");
		param = param.replace("prpCinsureds%5B1%5D.acceptLicenseDate=", "prpCinsureds%5B1%5D.acceptLicenseDate=");
		param = param.replace("isCheckRepeat%5B1%5D=0", "isCheckRepeat%5B1%5D=0");
		param = param.replace("configedRepeatTimes%5B1%5D=", "configedRepeatTimes%5B1%5D=");
		param = param.replace("repeatTimes%5B1%5D=", "repeatTimes%5B1%5D=");
		param = param.replace("prpCinsureds%5B1%5D.unifiedSocialCreditCode=","prpCinsureds%5B1%5D.unifiedSocialCreditCode=");
		param = param.replace("idCardCheckInfo%5B1%5D.insuredcode=", "idCardCheckInfo%5B1%5D.insuredcode=");
		param = param.replace("idCardCheckInfo%5B1%5D.insuredFlag=", "idCardCheckInfo%5B1%5D.insuredFlag=");
		param = param.replace("idCardCheckInfo%5B1%5D.mobile=", "idCardCheckInfo%5B1%5D.mobile=");
		param = param.replace("idCardCheckInfo%5B1%5D.idcardCode=", "idCardCheckInfo%5B1%5D.idcardCode=");
		param = param.replace("idCardCheckInfo%5B1%5D.name=", "idCardCheckInfo%5B1%5D.name=");
		param = param.replace("idCardCheckInfo%5B1%5D.nation=", "idCardCheckInfo%5B1%5D.nation=");
		param = param.replace("idCardCheckInfo%5B1%5D.birthday=", "idCardCheckInfo%5B1%5D.birthday=");
		param = param.replace("idCardCheckInfo%5B1%5D.sex=", "idCardCheckInfo%5B1%5D.sex=");
		param = param.replace("idCardCheckInfo%5B1%5D.address=", "idCardCheckInfo%5B1%5D.address=");
		param = param.replace("idCardCheckInfo%5B1%5D.issure=", "idCardCheckInfo%5B1%5D.issure=");
		param = param.replace("idCardCheckInfo%5B1%5D.validStartDate=", "idCardCheckInfo%5B1%5D.validStartDate=");
		param = param.replace("idCardCheckInfo%5B1%5D.validEndDate=", "idCardCheckInfo%5B1%5D.validEndDate=");
		param = param.replace("idCardCheckInfo%5B1%5D.samCode=", "idCardCheckInfo%5B1%5D.samCode=");
		param = param.replace("idCardCheckInfo%5B1%5D.samType=", "idCardCheckInfo%5B1%5D.samType=");
		param = param.replace("idCardCheckInfo%5B1%5D.flag=", "idCardCheckInfo%5B1%5D.flag=");
		param = param.replace("prpCinsureds%5B1%5D.id.serialNo=1", "prpCinsureds%5B3%5D.id.serialNo=1");
		param = param.replace("prpCinsureds%5B1%5D.insuredType=1", "prpCinsureds%5B3%5D.insuredType=1");
		param = param.replace("prpCinsureds%5B1%5D.insuredNature=3", "prpCinsureds%5B3%5D.insuredNature=1");
		param = param.replace("prpCinsureds%5B1%5D.unitType=", "prpCinsureds%5B3%5D.unitType=");
		param = param.replace("prpCinsureds%5B1%5D.identifyType=01", "prpCinsureds%5B3%5D.identifyType=01");
		param = param.replace("prpCinsureds%5B1%5D.email=", "prpCinsureds%5B3%5D.email=");
		param = param.replace("prpCinsureds%5B1%5D.phoneNumber=13520030193", "prpCinsureds%5B3%5D.phoneNumber=");
		param = param.replace("prpCinsureds%5B1%5D.sex=", "prpCinsureds%5B3%5D.drivingYears=");
		param = param.replace("prpCinsureds%5B1%5D.postCode=100000", "prpCinsureds%5B3%5D.postCode=");
		param = param.replace("prpCinsureds%5B1%5D.versionNo=0", "prpCinsureds%5B3%5D.versionNo=3");
		param = param.replace("prpCinsureds%5B1%5D.auditStatus=", "prpCinsureds%5B3%5D.auditStatus=2");
		param = param.replace("prpCinsureds%5B1%5D.countryCod==", "prpCinsureds%5B3%5D.countryCode=CHN");
		param = param.replace("prpCinsureds%5B1%5D.flag=", "prpCinsureds%5B3%5D.flag=");
		param = param.replace("prpCinsureds%5B1%5D.appendPrintName=", "prpCinsureds%5B3%5D.drivingCarType=");
		param = param.replace("prpCinsureds%5B1%5D.drivingCarType=", "prpCinsureds%5B3%5D.appendPrintName=");
		param = param.replace("prpCinsureds%5B1%5D.causetroubleTimes=0", "prpCinsureds%5B3%5D.causetroubleTimes=");
		param = param.replace("prpCinsureds%5B1%5D.acceptLicenseDate=", "prpCinsureds%5B3%5D.acceptLicenseDate=");
		param = param.replace("isCheckRepeat%5B1%5D0=", "isCheckRepeat%5B3%5D=");
		param = param.replace("configedRepeatTimes%5B1%5D=", "configedRepeatTimes%5B3%5D=");
		param = param.replace("repeatTimes%5B1%5D=", "repeatTimes%5B3%5D=");
		param = param.replace("prpCinsureds%5B1%5D.unifiedSocialCreditCode=","prpCinsureds%5B3%5D.unifiedSocialCreditCode=");
		param = param.replace("idCardCheckInfo%5B1%5D.insuredcode=", "idCardCheckInfo%5B3%5D.insuredcode=");
		param = param.replace("idCardCheckInfo%5B1%5D.insuredFlag=", "idCardCheckInfo%5B3%5D.insuredFlag=");
		param = param.replace("idCardCheckInfo%5B1%5D.mobile=", "idCardCheckInfo%5B3%5D.mobile=");
		param = param.replace("idCardCheckInfo%5B1%5D.idcardCode=", "idCardCheckInfo%5B3%5D.idcardCode=");
		param = param.replace("idCardCheckInfo%5B1%5D.name=", "idCardCheckInfo%5B3%5D.name=");
		param = param.replace("idCardCheckInfo%5B1%5D.nation=", "idCardCheckInfo%5B3%5D.nation=");
		param = param.replace("idCardCheckInfo%5B1%5D.birthday=", "idCardCheckInfo%5B3%5D.birthday=");
		param = param.replace("idCardCheckInfo%5B1%5D.sex=", "idCardCheckInfo%5B3%5D.sex=");
		param = param.replace("idCardCheckInfo%5B1%5D.addres==", "idCardCheckInfo%5B3%5D.address=");
		param = param.replace("idCardCheckInfo%5B1%5D.issure=", "idCardCheckInfo%5B3%5D.issure=");
		param = param.replace("idCardCheckInfo%5B1%5D.validStartDate=", "idCardCheckInfo%5B3%5D.validStartDate=");
		param = param.replace("idCardCheckInfo%5B1%5D.validEndDate=", "idCardCheckInfo%5B3%5D.validEndDate=");
		param = param.replace("idCardCheckInfo%5B1%5D.samCode=", "idCardCheckInfo%5B3%5D.samCode=");
		param = param.replace("idCardCheckInfo%5B1%5D.samType=", "idCardCheckInfo%5B3%5D.samType=");
		param = param.replace("idCardCheckInfo%5B1%5D.flag=", "idCardCheckInfo%5B3%5D.flag=");
		param = param.replace("prpCinsureds%5B1%5D.mobile=13520030193", "iauditStatus=2");
		param = param.replace("phoneNumber%5B0%5D=18911***071", "");
		param = param.replace("hidden_index_insured=2", "iversionNo=3");
		param = param.replace("_insuredName=%D6%EC%BC%D1%BC%D1", "_insuredName=");
		param = param.replace("_sex=0", "_sex=1");
		param = param.replace("phoneNumber%5B1%5D=1***518", "");
		param = param.replace("reLoadFlag%5B1%5D=true", "");
		param = param.replace("isCheckRepeat%5B1%5D=0", "isCheckRepeat%5B3%5D=");
		param = param.replace("prpCinsureds%5B1%5D.countryCode=", "prpCinsureds%5B3%5D.countryCode=CHN");
		param = param.replace("idCardCheckInfo%5B1%5D.address=", "idCardCheckInfo%5B3%5D.address=");
		param = param + "&hidden_index_insured=2";
		param = param + "&iinsuredType=%B8%F6%C8%CB";
		param = param + "&iunitType=";
		param = param + "&iidentifyType=%C9%ED%B7%DD%D6%A4";
		param = param + "&iemail=";
		param = param + "&iphoneNumber=";
		param = param + "&prpCinsureds%5B2%5D.sex=1";
		param = param + "&iversionNo=3";
